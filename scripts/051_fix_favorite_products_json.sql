-- Fix favorite_products to return structured JSON instead of string
...
RETURNS TABLE (
  ...
  favorite_products JSONB  -- Changed from TEXT to JSONB
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  WITH customer_stats AS (
    SELECT 
      ...
      -- Return structured JSON with product_id, name, and quantity
      COALESCE(
        JSONB_AGG(
          JSONB_BUILD_OBJECT(
            'product_id', product_summary.product_id,
            'name', product_summary.product_name,
            'quantity', product_summary.order_count
          )
          ORDER BY product_summary.order_count DESC
        ) FILTER (WHERE product_summary.product_id IS NOT NULL),
        '[]'::JSONB
      ) AS favorite_products
    FROM public.customers c
    LEFT JOIN public.orders o ON o.customer_id = c.id AND o.status != 'cancelled'
    LEFT JOIN LATERAL (
      -- Aggregate order items per product per customer
      SELECT 
        oi.product_id,
        oi.product_name,
