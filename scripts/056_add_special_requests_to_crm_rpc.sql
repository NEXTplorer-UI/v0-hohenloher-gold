-- Add special_requests field to crm_customers_search RPC
-- This allows customer notes/special requests to be displayed in customer management
...
RETURNS TABLE (
  ...
  tags TEXT[],
  special_requests TEXT  -- Added special_requests field
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  ...
  customer_stats AS (
    SELECT 
      ...
      c.special_requests,  -- Added special_requests to CTE
    ...
    GROUP BY 
      ...
      c.favorite_categories, c.special_requests  -- Added to GROUP BY
  )
  SELECT 
    cs.id,
    cs.first_name,
    ...
    cs.special_requests  -- Added special_requests to final SELECT
