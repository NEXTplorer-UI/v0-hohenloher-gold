  ...
  const bankDetailsSection =
    ...
    <div class="highlight-box">
      ...
      <div class="info-box" style="margin-top: 15px; background: #fef3c7; border-left: 4px solid #d4af37;">
        <p style="margin: 0; font-size: 14px;">
          <strong>${copy.orderConfirmation.bankPaymentDeadline}</strong> ${copy.orderConfirmation.bankPaymentDeadlineText} <strong>${new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toLocaleDateString("de-DE")}</strong>
        </p>
      </div>
    </div>
  `
    ...
    <div class="info-box">
      ...
      ${
        vars.pickupLocation && vars.deliveryMethod === "pickup"
          ? `<p><strong>Abholort:</strong> {{pickupLocation}}</strong></p>`
          : ""
      }
    </div>
...
export function pickupReminderContent(vars: TemplateVars, customCopy?: EmailCopyType): string {
  ...
  const itemsList = vars.orderItems
    ? (vars.orderItems as any[])
        .map(
          (item) =>
            `<div class="data-row">
          <span class="data-label">${item.product_name} - ${item.quantity} ${item.unit || "Stück"}</span>
          <span class="data-value">${item.unit_price ? `${(item.unit_price * item.quantity).toFixed(2)} €` : ""}</span>
        </div>`,
        )
        .join("")
    : ""
  ...
  const template = `
    ...
    ${itemsList ? `<div class="data-section"><h3>Ihre Bestellung:</h3>${itemsList}<div class="data-row" style="border-top: 2px solid #a16207; margin-top: 10px; padding-top: 10px;"><span class="data-label"><strong>Gesamtbetrag:</strong></span><span class="data-value"><strong>{{totalAmount}}</strong></span></div></div>` : ""}
...
export function readyForPickupContent(vars: TemplateVars, customCopy?: EmailCopyType): string {
  ...
  // Determine pickup time display based on location
  const isZentrallager = vars.pickupLocation && vars.pickupLocation.toLowerCase().includes("zentrallager")
  const pickupTimeDisplay = isZentrallager && vars.pickupTime ? vars.pickupTime : "Nach Vereinbarung mit Verteilperson"
  ...
  const template = `
    ...
    <div class="highlight-box">
      ...
      <div class="data-row">
        <span class="data-label">${copy.readyForPickup.pickupDate}</span>
        <span class="data-value">{{pickupDate}}</span>
      </div>
      <div class="data-row">
      ...
      <div class="data-row">
        <span class="data-label">Abholzeit:</span>
        <span class="data-value">${pickupTimeDisplay}</span>
      </div>
    </div>
    
    <p>${copy.readyForPickup.reminder}</p>
...
export function complaintConfirmationContent(vars: TemplateVars, customCopy?: EmailCopyType): string {
  const copy = customCopy || emailCopy
  return `
    <p>Hallo${vars.customerName ? ` ${vars.customerName}` : ""},</p>
    <p>vielen Dank für Ihre Nachricht. Wir haben Ihre Reklamation zu Bestellung <strong>${vars.orderNumber || ""}</strong> erhalten und werden uns schnellstmöglich darum kümmern.</p>
    <p>Ein Mitglied unseres Teams wird sich in Kürze mit Ihnen in Verbindung setzen.</p>
    <p>Mit freundlichen Grüßen,<br>Ihr Südfrüchte Hohenlohe Team</p>
  `
}

// Abgeholt-Bestätigung Content
export function orderPickedUpContent(vars: TemplateVars, customCopy?: EmailCopyType): string {
  const copy = customCopy || emailCopy
  return `
    <p>Hallo${vars.customerName ? ` ${vars.customerName}` : ""},</p>
    
    <p>vielen Dank, dass Sie Ihre Bestellung <strong>${vars.orderNumber || ""}</strong> bei uns abgeholt haben!</p>
    
    <div style="background: #fef3c7; border-left: 4px solid #a16207; padding: 15px; margin: 20px 0; border-radius: 4px;">
      <p style="margin: 0; color: #78350f;"><strong>Wir hoffen, Ihnen hat alles geschmeckt!</strong></p>
      <p style="margin: 10px 0 0 0; color: #78350f;">Ihre Meinung ist uns wichtig. Haben Sie Feedback oder Anregungen? Schreiben Sie uns gerne eine E-Mail.</p>
    </div>
    
    <p>Wir freuen uns darauf, Sie bald wieder bei uns begrüßen zu dürfen.</p>
    
    <p>Mit sonnigen Grüßen,<br>Ihr Südfrüchte Hohenlohe Team</p>
  `
}
